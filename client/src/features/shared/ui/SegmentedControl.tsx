...
export function SegmentedControl<T extends string>({ options, value, onChange, label }: SegmentedControlProps<T>) {
  return (
    <div
      role="radiogroup"
      aria-label={label}
      className="inline-flex gap-1 rounded-2xl bg-night-950/70 p-1 shadow-[inset_0_2px_4px_rgb(0_0_0/0.45)] ring-1 ring-white/6"
    >
      {options.map(({ value: optionValue, label: optionLabel, icon: Icon }) => {
        ...
        return (
          <button
            ...
            className={cn(
              'inline-flex h-10 items-center gap-2 rounded-xl px-4 font-display text-sm font-bold transition-colors',
